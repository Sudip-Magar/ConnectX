<div>
    @include('common.message')


    <!-- Main Content -->
    <main class="max-w-[85%] mx-auto flex flex-col md:flex-row gap-6 pt-3 px-4 relative">

        <!-- Left Sidebar -->
        <aside class="hidden md:flex flex-col gap-4 w-1/4 sticky top-21 self-start">
            <div class="bg-white rounded-lg p-4 shadow">
                <h2 class="font-bold mb-2">Shortcuts</h2>
                <ul class="flex flex-col gap-2 text-gray-700">
                    <li><a href="#" class="hover:text-blue-500">Groups</a></li>
                    <li><a href="#" class="hover:text-blue-500">Events</a></li>
                    <li><a href="#" class="hover:text-blue-500">Saved</a></li>
                </ul>
            </div>
        </aside>

        <!-- Feed -->
        <div class="flex-1 flex flex-col gap-6">
            <livewire:menus.post />
        </div>

        <!-- Right Sidebar -->
        <aside class="hidden md:flex flex-col gap-4 w-1/4 sticky top-21 self-start">
            <div class="bg-white rounded-lg p-4 shadow h-100">
                <h2 class="font-bold mb-2">People You May Know</h2>
                <livewire:menus.people>
            </div>

            <div class="bg-white rounded-lg p-4 shadow h-100">
                message
            </div>
        </aside>
    </main>
</div>
